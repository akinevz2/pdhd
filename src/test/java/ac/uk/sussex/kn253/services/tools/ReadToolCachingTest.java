...
class ReadToolCachingTest {
    ...
    void readFolderManifestToolCachesManifestToProjectKnowledge(@TempDir final Path tempDir) throws Exception {
        ...
        if (project != null) {
            ...
            if (cached != null) {
                ...
                assertTrue(cached.getJsonContent().contains("manifest") || cached.getJsonContent().contains("folder"),
    ...
    void readToolsCacheContextPerProject(@TempDir final Path tempDir) throws Exception {
        ...
        toolService.execute(
                request("read_file",
                        "{\"projectDirectory\":\"" + escape(projectA) + "\",\"filePath\":\"file.txt\"}"),
                null);
        toolService.execute(
                request("read_file",
                        "{\"projectDirectory\":\"" + escape(projectB) + "\",\"filePath\":\"file.txt\"}"),
                null);
