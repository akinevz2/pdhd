...
public class AssistantToolRegistry {
    ...
    public Map<String, Function<ToolExecutionRequest, String>> dispatchMap() {
        ...
        dispatch.put("listDirectoryContents",
                request -> workspaceContextTools.listDirectoryContents(stringArg(request, "directoryPath")));

        dispatch.put("change_working_directory",
                request -> workspaceContextTools.changeWorkingDirectory(stringArg(request, "directoryPath")));
        ...
        dispatch.put("summarize_path", request -> workspaceContextTools.summarizePath(stringArg(request, "path")));

        dispatch.put("readFile", request -> readFileTools.readFile(stringArg(request, "path")));

        dispatch.put("searchWeb",
                request -> webSearchTools.searchWeb(stringArg(request, "query"), integerArg(request, "maxResults")));
