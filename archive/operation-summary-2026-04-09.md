...
## Maintenance Correction (2026-04-15)

This historical note has been corrected where later code cleanup made some of
its configuration claims stale. The startup-simplification summary remains the
same, but the `OllamaConfig` table and checked-in base URL defaults below now
...
| `timeoutSeconds()`      | `pdhd.ollama.timeout-seconds`       | `300`             |
...
## 4. Checked-in Base URL Defaults Are Profile-Specific

The current checked-in properties are not uniform across all profiles.

- The dev profile uses `http://host.docker.internal:11434`.
- The default profile uses `http://localhost:11434`.
- The test profile uses `http://ws-vision.local:11434`.

**Evidence** — `src/main/resources/application.properties`:

```properties
%dev.pdhd.ollama.base-url=http://host.docker.internal:11434
pdhd.ollama.base-url=http://localhost:11434
```

**Evidence** — `src/test/resources/application-test.properties`:

```properties
pdhd.ollama.base-url=http://ws-vision.local:11434
```

---

## 5. Model Defaults Use `gemma4` in Code and `gemma4:latest` in Properties

The `OllamaConfig` accessor default remains `gemma4`, while the checked-in
properties use `gemma4:latest` for runtime configuration.

**Evidence** — `src/main/resources/application.properties`:

```properties
%dev.pdhd.ollama.model-name=gemma4:latest
...
The `OllamaConfig.modelName()` accessor still carries `@WithDefault("gemma4")`,
which is the compile-time fallback when the property is absent.
...
1. Keep downstream docs aligned with the current checked-in defaults for base URL, timeout, and model-name resolution order.
2. Decide whether the checked-in `gemma4:latest` property values should remain intentionally different from the typed `gemma4` fallback in `OllamaConfig`.
