...
This file is the canonical recommendations document for implementation planning.
It consolidates the previously split recommendations files.
...
To strengthen both practical deployment and academic quality, implementation
should proceed as a staged roadmap with measurable checkpoints.

### 6. Calibrate scope with explicit risk management

Implementation planning should distinguish between immediate, medium-term, and
exploratory work. A recommended sequencing model is:
...
Each phase should include entry/exit criteria so conclusions remain grounded in
delivered capability rather than intent.

### Implementation progress note (2026-04-14)

Recommendations 1, 2, 3, 4, 5, 7, 8, and 9 have been implemented and moved to
...
This addendum converts the recommendations above into an implementation
sequence with explicit outputs and acceptance checks.

#### Phase 1: Reliability Baseline (2 weeks)

Goal: ensure core tool execution and API boundaries are predictable before
extending features.
...
- Baseline metrics capture script (P50/P95 latency, tool failure rate,
...
Goal: extend capability only after reliability and measurement controls are in
place.
...
#### Phase 5: Configuration API Test Coverage (1 week)

Goal: ensure every HTTP endpoint that modifies or reads application
configuration is covered by at least one `@QuarkusTest` unit test so that
regressions in configuration persistence or validation are caught before
benchmark runs.

Deliverables:

- `@QuarkusTest` tests for all `MenuResource` configuration operations
  (`GET /api/menu/ollama`, `POST /api/menu/ollama`,
  `POST /api/menu/ollama/runtime/provider`,
  `GET /api/menu/ollama/status`, `GET /api/menu/ollama/models`).
- `@QuarkusTest` test for `ToolTelemetryResource` (`GET /api/telemetry`)
  verifying schema-version field and invocation-count aggregation.
- Input-validation boundary tests: missing provider, blank model name,
  empty base URL for EXTERNAL provider.

Acceptance criteria:

- All new tests pass under `./mvnw -Dtest=MenuResourceConfigApiTest,ToolTelemetryResourceTest test`.
- No existing test is broken.
- Validation rejection paths return the appropriate HTTP status (`400`).

Evidence to capture:

- Test class file paths.
- Decision-log entry recording coverage gap closure.

#### Cross-phase operating rules

- Do not merge feature work that reduces telemetry continuity.
- Record a brief decision log entry for each change to dispatch policy,
  schema contract, or cache semantics.
